...
public partial class UIPartyHUD : MonoBehaviour {
    ...
    void Update() {
        ...
        if (player.InParty()) {
            ...
            var members = player.InParty() ? party.members.Where(m => m != player.name).ToList() : new List<string>();
            ...
            for (int i = 0; i < members.Count; ++i) {
                var slot = memberContent.GetChild(i).GetComponent<UIPartyHUDMemberSlot>();
                ...
                if (Player.onlinePlayers.ContainsKey(memberName)) {
                    var member = Player.onlinePlayers[memberName];
